...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `
    <div  class="single-product bg-light m-3 p-4 rounded-3" style="height:420px ">
      <div>
        <img class="product-image rounded-3"  src=${image}></img>
      </div>
      <h3>${product.title}</h3>
      <h4 class="my-4 text-secondary fw-bold">Category: ${product.category}</h4>
      <div class="d-flex justify-content-between">
        <h4 class="text-info bg-dark p-2 rounded-3">Counting: ${product.rating.count} </h4>
        <h4 class='text-dark bg-warning p-2 rounded-3'>Rating: ${product.rating.rate} </h4>
      </div>
      <h2>Price: $<span class="fw-bold">${product.price}</span></h2>
      <div class="my-4" >
        <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success" style="font-size:16px">add to cart</button>
        <button id="details-btn" class="btn btn-danger" style="font-size:16px"  type="button" data-bs-toggle="modal" data-bs-target="#exampleModal">Details</button>
      </div>
    </div>  `;
